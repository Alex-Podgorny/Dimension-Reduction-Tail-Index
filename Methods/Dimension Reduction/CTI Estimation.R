...
Psi_function <- function(X, y, X0, B, interm_lvl, bandwidth) {
  ...
  mean(local_Hill(X, y, X[X0, ], B, interm_lvl, bandwidth), na.rm = TRUE)
}
...
CTI <- function(X, y, X0, q, interm_lvl, bandwidth, num_init_evals = 100, num_starts = 3) {
  
  # Define the objective function
  objective_fn <- function(B) {
    B <- qr.Q(qr(matrix(B, nrow = p, ncol = q)))
    Psi_function(X, y, X0, B, interm_lvl, bandwidth)
  }
