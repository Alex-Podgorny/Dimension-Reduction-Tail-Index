...
for(model in models){
  ...
  for (seed_dir in seed_dirs) {
    ...
    if (length(Errors$Gardes$Bhat) != 0) {
      ...
      # Calculate the mean matrix for each method across all seeds
      means_Bhat_p30 <- lapply(errors_Bhat_p30, function(method_errors) {
      ...
      means_gamma_p30 <- lapply(errors_gamma_p30, function(method_errors) {
      ...
      # Create heatmaps for each method with p = 30
      
      for (method in names(means_gamma_p30)) {
        # Convert the mean error matrix to a data frame for ggplot2
        if (!is.null(means_Bhat_p30[[method]])) {
          mean_Bhat_df_long <- data.frame(
            Var1 = rep(seq_len(nrow(means_Bhat_p30[[method]])), ncol(means_Bhat_p30[[method]])),
            Var2 = rep(seq_len(ncol(means_Bhat_p30[[method]])), each = nrow(means_Bhat_p30[[method]])),
            value = as.vector(means_Bhat_p30[[method]])
          )
        ...
        mean_gamma_df_long <- data.frame(
          Var1 = rep(seq_len(nrow(means_gamma_p30[[method]])), ncol(means_gamma_p30[[method]])),
          Var2 = rep(seq_len(ncol(means_gamma_p30[[method]])), each = nrow(means_gamma_p30[[method]])),
          value = as.vector(means_gamma_p30[[method]])
        )
      ...
      # Find the optimal hyperparameters for each method (p = 4)
      min_params_gamma_p30 <- lapply(means_gamma_p30, function(matrix) {
        which(matrix == min(matrix), arr.ind = TRUE)
      })
      ...
      for (method in names(means_Bhat_p30)) {
        # Get the optimal alpha and h indices for Bhat and gamma
        alpha_idx <- min_params_gamma_p30[[method]][1]
        h_idx <- min_params_gamma_p30[[method]][2]
        
        # Extract Bhat errors for optimal parameters and add to the combined data frame
        errors_Bhat_best <- unlist(lapply(errors_Bhat_p30[[method]], function(matrix) matrix[alpha_idx, h_idx]))
        all_errors_Bhat <- rbind(all_errors_Bhat, data.frame(value = errors_Bhat_best, method = method, error_type = "Bhat"))
        
        # Extract gamma errors for optimal parameters and add to the combined data frame
        errors_gamma_best <- unlist(lapply(errors_gamma_p30[[method]], function(matrix) matrix[alpha_idx, h_idx]))
        all_errors_gamma <- rbind(all_errors_gamma, data.frame(value = errors_gamma_best, method = method, error_type = "Gamma"))
      }
